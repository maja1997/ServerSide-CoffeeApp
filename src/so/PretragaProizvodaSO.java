 ...
 * @author Maja
 */
public class PretragaProizvodaSO extends OpstaSistemskaOperacija{

    public PretragaProizvodaSO() {
        super();
    }
    ...
    protected ServerskiOdgovor izvrsiOperaciju(IObjekat domenskiObjekat) throws Exception {
        ...
        try {
          ...
          so.setUspesno(true);
          so.setOdgovor(lista);
          so.setPoruka("Uspesno pronadjeni proizvodi");

        } catch (Exception e) {
            Logger.getLogger(PretragaRacunaSO.class.getName()).log(Level.SEVERE, null, e);

            throw new Exception("Nije pronadjena lista proizvoda");
        }
        return so;
    }
}
